...
const iconEnd = appStr.indexOf('];', iconStart) + 2;

// regex to strip double quotes off keys so we don't cause eslint errors:
const newIconStr = 'const techStackIcons = ' + JSON.stringify(icons, null, 2) + ';';
// let's actually just leave stringify and see.

appStr = appStr.substring(0, iconStart) + newIconStr + appStr.substring(iconEnd);

const sgStart = appStr.indexOf('const skillGroups = [');
const sgEnd = appStr.indexOf('];', sgStart) + 2;
